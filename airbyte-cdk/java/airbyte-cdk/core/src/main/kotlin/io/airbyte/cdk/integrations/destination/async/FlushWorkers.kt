...
class FlushWorkers(
    ...
    private fun flush(
        ...
        val flushTask = Runnable {
            logger.info {
                "Flush Worker (${humanReadableFlushWorkerId(flushWorkerId)}) -- Worker picked up work."
            }
            try {
                logger.info {
                ...
                bufferDequeue.take(desc, destinationFlushFunction.optimalBatchSizeBytes).use { batch
                    ...
                    val stateIdToCount: Map<Long, Long> =
                        batch
                            .getData()
                            .stream()
                    ...
                    logger.info {
                        "Flush Worker (${humanReadableFlushWorkerId(
                                flushWorkerId,
                            )}) -- Batch contains: ${batch.getData().size} records, ${airbyteFileUtils.byteCountToDisplaySize(
                                batch.sizeInBytes,
                            )} bytes."
                    }
                    ...
                    destinationFlushFunction.flush(
                        desc,
                        batch.getData().stream().map(StreamAwareQueue.MessageWithMeta::message),
                    )
                    batch.flushStates(stateIdToCount, outputRecordCollector)
                }
            } catch (e: Exception) {
                logger.error(e) {
                    "Flush Worker (${humanReadableFlushWorkerId(flushWorkerId)}) -- flush worker error: "
                }
                flushFailure.propagateException(e)
            } finally {
